...
class Transformer(nn.Module):
        ...
        for _ in range(max_length):
            ...
            for seq, score in beam:
                ...
                for next_token in range(log_probs.size(1)):
                    ...
                    new_seq = torch.cat([seq, next_token_tensor], dim=1)
                    new_score = score + log_probs[0, next_token]
    ...
    def __contrastive_decode(self, src_input: torch.Tensor, max_length: int, k: int = 5, alpha: float = 0.6, **kwargs) -> torch.Tensor:
        ...
        # Initialize the target sequence with SOS_token
        tgt_input = torch.full((batch_size, 1), SOS_token, dtype=torch.long, device=device)  # Shape: (1, 1)
        ...
        for _ in range(max_length):
            ...
            # Get the top k tokens
            topk_probs, topk_indices = probs.topk(k, dim=-1)  # Shape: (batch_size, k)
            topk_probs = topk_probs.squeeze(0)  # (k,)
            topk_indices = topk_indices.squeeze(0)      # (k,)

            # Prepare tensors for all candidates
            expanded_tgt_input = tgt_input.expand(k, tgt_input.size(1))  # Shape: (batch_size, k, seq_len)
            ...
            # Compute scores
            P_LM_v = topk_probs - (1 - alpha) * max_sim   # Shape: (k,)
            scores = alpha * P_LM_v   # Shape: (k,)
